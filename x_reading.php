...
$stmt->bind_param("is", $store_id, $today);
$stmt->execute();
$hourly_sales = $stmt->get_result();

// Handle X Reading submission
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_x_reading') {
    ...
    $insert_query = "INSERT INTO x_readings (
        store_id, cashier_id, reading_date, total_transactions, 
        total_sales, cash_sales, card_sales, gcash_sales, remarks
    ) VALUES (?, ?, CURDATE(), ?, ?, ?, ?, ?, ?)";
    
    $stmt = $conn->prepare($insert_query);
    $stmt->bind_param("iiidddds", 
        $store_id, $cashier_id, $total_transactions, 
        $total_sales, $cash_sales, $card_sales, $gcash_sales, $remarks
    );
    
    if ($stmt->execute()) {
        $_SESSION['success'] = "X Reading saved successfully.";
    } else {
        $_SESSION['error'] = "Error saving X Reading: " . $stmt->error;
    }
...
if (isset($_GET['download_pdf'])) {
    ...
    $dompdf->render();
    $dompdf->stream('x_reading_report.pdf');
    exit();
}
?>
...
<head>
    ...
    <title>X Reading - <?php echo htmlspecialchars($store['name']); ?></title>
            ...
            <div class="col-md-8">
                ...
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-0">X Reading - <?php echo htmlspecialchars($store['name']); ?></h5>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="row mb-4">
                            <div class="col-md-6">
                                <h6>Date: <?php echo date('F d, Y'); ?></h6>
                                <h6>Store: <?php echo htmlspecialchars($store['name']); ?></h6>
                            </div>
                            <div class="col-md-6 text-end">
                                <h6>Cashier: <?php echo htmlspecialchars($_SESSION['username']); ?></h6>
                                <h6>Time: <?php echo date('h:i A'); ?></h6>
                            </div>
                        </div>

                        <div class="d-flex justify-content-end mb-4">
                            <a href="?store_id=<?php echo $store_id; ?>&download_pdf=1" class="btn btn-success me-2">
                                <i class="bi bi-file-earmark-pdf"></i> Download X Reading Report
                            </a>
                        ...
                        <!-- Hourly Sales -->
                        <h5 class="mt-4 mb-3">Hourly Sales</h5>
                        <div class="table-responsive">
                            <table class->table table-bordered">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td><?php echo date('h:i A', strtotime($hour['hour'] . ':00')); ?></td>
                                        <td class="text-end"><?php echo number_format($hour['transactions']); ?></td>
                        ...
                        <form method="POST" class="mt-4">
                            <input type="hidden" name="action" value="save_x_reading">
                            <div class="mb-3">
                            ...
                            <div class="d-flex gap-2">
                                <button type="submit" class="btn btn-primary">
                                    <i class="bi bi-save"></i> Save X Reading
                                </button>
                                <button type="button" class="btn btn-secondary" onclick="window.print()">
                                    <i class="bi bi-printer"></i> Print
                                </button>
