...
$sales_query = "SELECT 
    ...
    FROM sales 
    WHERE store_id = ? AND DATE_FORMAT(created_at, '%Y-%m') = ?";
$stmt = $conn->prepare($sales_query);
$stmt->bind_param("is", $store_id, $current_month);
$stmt->execute();
...
$product_query = "SELECT 
    ...
    WHERE s.store_id = ? AND DATE_FORMAT(s.created_at, '%Y-%m') = ?
...
// Handle Y Reading submission
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_y_reading') {
    ...
    $insert_query = "INSERT INTO y_readings (
        store_id, cashier_id, reading_month, total_transactions, 
        total_sales, cash_sales, card_sales, gcash_sales, business_days, remarks
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    
    $stmt = $conn->prepare($insert_query);
    $stmt->bind_param("iisdddddis", 
        $store_id, $cashier_id, $current_month, $total_transactions, 
        $total_sales, $cash_sales, $card_sales, $gcash_sales, $business_days, $remarks
    );
    
    if ($stmt->execute()) {
        $_SESSION['success'] = "Y Reading saved successfully.";
    } else {
        $_SESSION['error'] = "Error saving Y Reading: " . $stmt->error;
    }
...
if (isset($_GET['download_pdf'])) {
    ...
    $dompdf->render();
    $dompdf->stream('y_reading_report.pdf');
    exit();
}
?>
...
<head>
    ...
    <title>Y Reading - <?php echo htmlspecialchars($store['name']); ?></title>
            ...
            <div class="col-md-10">
                ...
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-0">Y Reading - <?php echo htmlspecialchars($store['name']); ?></h5>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-end mb-4">
                            <a href="?store_id=<?php echo $store_id; ?>&download_pdf=1" class="btn btn-success me-2">
                                <i class="bi bi-file-earmark-pdf"></i> Download Y Reading Report
                            </a>
                        ...
                        <h5 class="mt-4 mb-3">Daily Sales Breakdown</h5>
                        <div class="table-responsive">
                            <table class="table table-bordered">
                                <thead>
                                    <tr>
                        ...
                        <form method="POST" class="mt-4 no-print">
                            <input type="hidden" name="action" value="save_y_reading">
                            <div class="mb-3">
                            ...
                            <div class="d-flex gap-2">
                                <button type="submit" class="btn btn-success">
                                    <i class="bi bi-save"></i> Save Y Reading
                                </button>
                                <button type="button" class="btn btn-secondary" onclick="window.print()">
                                    <i class="bi bi-printer"></i> Print
                                </button>
